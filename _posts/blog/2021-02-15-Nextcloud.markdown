...
직접 설치하여 운영할 수 있고 Office , Google Drive , Mattermost , Calendar 등 통합된 기능을 로컬 컴퓨터 또는 외부 파일 스토리지 호스팅에 사용할 수 있어 클라우드 스토리지 서비스에 적합합니다.
...
위 표는 Nextcloud는 소프트웨어 플랫폼에서 통합된 기능들을 나열하고 보여 주고 있습니다.
파일 저장 및 공유 서비스를 포함하여 문서 작업, 가상화, 클라우드 컴퓨팅 기술 등 하나의 플랫폼에서 사용할 수 있어서 효율적입니다. 
...
**클라우드의 기능**
 ...
 1. 통합된 기능으로 서버 운영에만 집중할 수 있어 효율성이 높아진다.

 1. 삭제한 파일도 기간 내 가져올 수 있으며, 파일 작업 시 자동으로 업데이트를 해줍니다.

**NAS 기능**

 1. NAS는 파일 관리에 최적이며 파일 관리 시스템의 대표적인 하나로 클라우드 서비스도 제공을 합니다.

 1. NAS는 네트워크 드라이브와 외부 저장 장치의 형태로만 사용되고 클라우드 서비스를 이용하면 동기화 방식이 기본으로 되어 용량이 늘어나면서 NAS와 같이 활용되는 목적으로 구축됩니다.

 1. NAS는 물리적인 구성 환경을 필요로 하고, RAID 구성으로 비용도 많이 드는데, 클라우드 서비스는 휴지통이나, 파일 히스토리 기능을 지원하여 복구가 수월하다고 생각되어 안정성이 보장된다고 생각이 됩니다.
...
* CentOS 7을 설치하여 성공적인 플랫폼을 제공하도록 구축했습니다.
<pre>
<code>
   # 최신 커널 업데이트를 진행합니다.
~]# yum update -y 
...
#### 3.2 PHP 관련 패키지 설치
* Nextcloud는 PHP 프로그래밍 언어를 사용하여 새로운 모듈을 만들고 싶으면 (현재 최신) 7.3 이상의 패키지를 설치합니다.
<pre>
<code>
...
* Nextcloud Server 다운로드 -> 다운로드 -> 서버 소유자용 아카이브 파일로 이동하여 tar.bz2 다운로드 하였습니다. 
2. GPG 공개키 오류로 인한 yum install 실패 시 해결하기 위한 조치사항입니다.
<pre>
<code>
...
3. RPM 기반의 패키지들은 GPG 공개키 기반의 디지털 서명과 검증을 통해서 각종 소스의 변조 유뮤를 검사해주고 무결성을 제공하여 허가된 사용자만 권한을 가질 수 있습니다. 
...
**[데스크탑 , 모바일 Nextcloud Install](https://nextcloud.com/install/)**

<p align="center"><img src="/assets/agent3.JPG" width="700px" height="400px">

1. 위 설치 링크로 들어가면 데스크탑과 모바일 전용 탐색기 2 종류의 에이전트가 있습니다.
...
7. 계정 접근이 활성화되면 윈도우에서 Nextcloud 앱이 실행됨을 확인할 수 있습니다.
...
2. 네트워크 기반의 클라우드 서비스로 액세스하는 사용자 데이터 흐름을 원할하게 해줍니다.
